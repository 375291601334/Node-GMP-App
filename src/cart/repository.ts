...
import { Cart, ICart, ICartItem } from './entities';
...
export const updateCartItems = async (cart: ICart, product: IProduct, count: number): Promise<ICart | null> => {
  const item = cart?.items.find((item) => (item.product as IProduct).id == product.id);

  try {
    if (item) {
      if (count == 0) {
        // delete item
        cart.items.pull({ _id: item._id as ICartItem['id'] });
        await cart.save();
      } else {
        // update item
        item.count = count;
